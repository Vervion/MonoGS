...
########################    NEW GNC LOSS UTILITIES    ####################################

def _gnc_enabled(config):
    gnc_cfg = config.get("GNC", None)
    return gnc_cfg is not None and gnc_cfg.get("enabled", False)


def _gnc_current_scale(config):
    # returns None if GNC not active
    if not _gnc_enabled(config):
        return None
    return float(config["GNC"].get("current_s", 0.0)) or None


def _cauchy_weight(residuals, s):
    """
    Cauchy IRLS weight:
        rho(r; s) = s^2 * log(1 + (r/s)^2)
        w(r; s)   = 1 / (1 + (r/s)^2)
    We detach residuals so that w is treated as fixed as in IRLS.
    """
    if s is None or s <= 0.0:
...
def gnc_loss_from_residuals(residuals, config):
    """
    Wraps a residual tensor into either:
        - standard L1 loss (if GNC disabled), or
        - Cauchy-IRLS weighted L2 (if GNC enabled).
    This is the 'thin wrapper' that replaces the basic loss.
    """
    s = _gnc_current_scale(config)
    if s is None:
        # original behavior: L1
        return torch.abs(residuals).mean()

    w = _cauchy_weight(residuals, s)
    return (w * (residuals ** 2)).mean()